...
function divide_time_interval($start_time, $end_time, $interval_minutes) {
    $start_time = strtotime($start_time);
    $end_time = strtotime($end_time);
    $interval_seconds = $interval_minutes * 60; // 10분을 초 단위로 변환
    $times = [];

    for ($current_time = $start_time; $current_time <= $end_time; $current_time += $interval_seconds) {
        $times[] = date("Y-m-d H:i:s", $current_time); // 시간을 "Y-m-d H:i" 형식으로 저장
    }

    if (end($times) !== date("Y-m-d H:i:s", $end_time)) {
        $times[] = date("Y-m-d H:i:s", $end_time);
    }

    return array_reverse($times);
}

function calculate_temperature($min, $max, $arrival_time, $current_time) {
    if (empty($current_time)) {
        return "";
    }
    $average = ($min + $max) / 2;
    $abs_diff = abs($min - $max) / 2;
    $time_diff = (strtotime($arrival_time) - strtotime($current_time)) / 86400; // 시간 차이를 일 단위로 계산
    $result = $average + $abs_diff * sin(80 * $time_diff);
